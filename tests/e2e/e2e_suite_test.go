...
import (
	...
	"github.com/europa/europa/v12/tests/e2e/upgrade"
)

const (
	// defaultManagerNetwork defines the network used by the upgrade manager
	defaultManagerNetwork = "europa-local"
...
// runNodeWithCurrentChanges builds a docker image using the current branch of the Europa repository.
...
func (s *IntegrationTestSuite) proposeUpgrade(name, target string) {
	...
	// if Europa is based on Evmos lower than v10.x.x no need to use the legacy proposal
