...
var _ = Describe("Recovery: Performing an IBC Transfer", Ordered, func() {
	...
	Describe("from a non-authorized chain", func() {
		BeforeEach(func() {
			...
			err := s.EuropaChain.App.(*app.Europa).ClaimsKeeper.SetParams(s.EuropaChain.GetContext(), params)
		...
		It("should transfer and not recover tokens", func() {
			s.SendAndReceiveMessage(s.pathOsmosisEuropa, s.IBCOsmosisChain, "uosmo", 10, sender, receiver, 1)

			nativeCoin := s.EuropaChain.App.(*app.Europa).BankKeeper.GetBalance(s.EuropaChain.GetContext(), senderAcc, constants.BaseDenom)
			Expect(nativeCoin).To(Equal(nativeCoin))
			ibcOsmo := s.EuropaChain.App.(*app.Europa).BankKeeper.GetBalance(s.EuropaChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
	...
	Describe("from an authorized, non-EVM chain (e.g. Osmosis)", func() {
		Describe("to a different account on Europa (sender != recipient)", func() {
			BeforeEach(func() {
				sender = s.IBCOsmosisChain.SenderAccount.GetAddress().String()
				receiver = s.EuropaChain.SenderAccount.GetAddress().String()
				senderAcc = sdk.MustAccAddressFromBech32(sender)
		...
		Describe("to the sender's own eth_secp256k1 account on Europa (sender == recipient)", func() {
			BeforeEach(func() {
				sender = s.IBCOsmosisChain.SenderAccount.GetAddress().String()
			...
			Context("with disabled recovery parameter", func() {
				BeforeEach(func() {
					...
					s.EuropaChain.App.(*app.Europa).RecoveryKeeper.SetParams(s.EuropaChain.GetContext(), params) //nolint:errcheck
				})

				It("should not transfer or recover tokens", func() {
					s.SendAndReceiveMessage(s.pathOsmosisEuropa, s.IBCOsmosisChain, coinOsmo.Denom, coinOsmo.Amount.Int64(), sender, receiver, 1)
					nativeCoin := s.EuropaChain.App.(*app.Europa).BankKeeper.GetBalance(s.EuropaChain.GetContext(), senderAcc, constants.BaseDenom)
					Expect(nativeCoin).To(Equal(nativeCoin))
					ibcOsmo := s.EuropaChain.App.(*app.Europa).BankKeeper.GetBalance(s.EuropaChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
					Expect(ibcOsmo).To(Equal(sdk.NewCoin(teststypes.UosmoIbcdenom, coinOsmo.Amount)))
			...
			Context("with a sender's claims record", func() {
				Context("without completed actions", func() {
					BeforeEach(func() {
						...
						s.EuropaChain.App.(*app.Europa).ClaimsKeeper.SetClaimsRecord(s.EuropaChain.GetContext(), senderAcc, claim)

						// update the escrowed account balance to maintain the invariant
						err := testutil.FundModuleAccount(s.EuropaChain.GetContext(), s.EuropaChain.App.(*app.Europa).BankKeeper, claimstypes.ModuleName, coins)
						s.Require().NoError(err)
					})
				...
				Context("with completed actions", func() {
					...
					It("should transfer tokens to the recipient and perform recovery", func() {
						...
						balanceEscrow := s.EuropaChain.App.(*app.Europa).BankKeeper.GetBalance(s.EuropaChain.GetContext(), transfertypes.GetEscrowAddress("transfer", "channel-0"), constants.BaseDenom)
						Expect(balanceEscrow).To(Equal(nativeCoin))
						ibcOsmo := s.EuropaChain.App.(*app.Europa).BankKeeper.GetBalance(s.EuropaChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
						Expect(ibcOsmo.IsZero()).To(BeTrue())

						// Relay both packets that were sent in the ibc_callback
						err := s.pathOsmosisEuropa.RelayPacket(CreatePacket("10000", constants.BaseDenom, sender, receiver, "transfer", "channel-0", "transfer", "channel-0", 1, timeout))
						s.Require().NoError(err)
						err = s.pathOsmosisEuropa.RelayPacket(CreatePacket("10", "transfer/channel-0/uosmo", sender, receiver, "transfer", "channel-0", "transfer", "channel-0", 2, timeout))
						s.Require().NoError(err)
					...
					It("should not claim/migrate/merge claims records", func() {
						...
						claimAfter, _ := s.EuropaChain.App.(*app.Europa).ClaimsKeeper.GetClaimsRecord(s.EuropaChain.GetContext(), senderAcc)
			...
			Context("without a sender's claims record", func() {
				...
				When("recipient has additional ibc vouchers that originated from other chains", func() {
					BeforeEach(func() {
						...
						// Send uatom from Cosmos to Europa
						s.SendAndReceiveMessage(s.pathCosmosEuropa, s.IBCCosmosChain, coinAtom.Denom, coinAtom.Amount.Int64(), s.IBCCosmosChain.SenderAccount.GetAddress().String(), receiver, 1)

						params.EnableRecovery = true
						s.EuropaChain.App.(*app.Europa).RecoveryKeeper.SetParams(s.EuropaChain.GetContext(), params) //nolint:errcheck
					})
					It("should not recover tokens that originated from other chains", func() {
						// Send uosmo from Osmosis to Europa
						s.SendAndReceiveMessage(s.pathOsmosisEuropa, s.IBCOsmosisChain, "uosmo", 10, sender, receiver, 1)
						...
						ibcAtom := s.EuropaChain.App.(*app.Europa).BankKeeper.GetBalance(s.EuropaChain.GetContext(), senderAcc, teststypes.UatomIbcdenom)
						Expect(ibcAtom).To(Equal(sdk.NewCoin(teststypes.UatomIbcdenom, coinAtom.Amount)))

						// Repeat transaction from Osmosis to Europa
						s.SendAndReceiveMessage(s.pathOsmosisEuropa, s.IBCOsmosisChain, "uosmo", 10, sender, receiver, 2)

						timeout = uint64(s.EuropaChain.GetContext().BlockTime().Add(time.Hour * 4).Add(time.Second * -20).UnixNano())
						err = s.pathOsmosisEuropa.RelayPacket(CreatePacket("10", "transfer/channel-0/uosmo", sender, receiver, "transfer", "channel-0", "transfer", "channel-0", 3, timeout))
						s.Require().NoError(err)
				...
				When("recipient has additional non-native ibc vouchers that originated from senders chains", func() {
					BeforeEach(func() {
						params := types.DefaultParams()
						params.EnableRecovery = false
						err := s.EuropaChain.App.(*app.Europa).RecoveryKeeper.SetParams(s.EuropaChain.GetContext(), params)
						s.Require().NoError(err)
						...
						transferMsg := transfertypes.NewMsgTransfer(s.pathOsmosisEuropa.EndpointA.ChannelConfig.PortID, s.pathOsmosisEuropa.EndpointA.ChannelID, sdk.NewCoin(teststypes.UatomIbcdenom, sdk.NewInt(10)), sender, receiver, timeoutHeight, 0, "")
						...
						packet := channeltypes.NewPacket(transfer.GetBytes(), 1, s.pathOsmosisEuropa.EndpointA.ChannelConfig.PortID, s.pathOsmosisEuropa.EndpointA.ChannelID, s.pathOsmosisEuropa.EndpointB.ChannelConfig.PortID, s.pathOsmosisEuropa.EndpointB.ChannelID, timeoutHeight, 0)
						// Receive message on the europa side, and send ack
						err = s.pathOsmosisEuropa.RelayPacket(packet)
						s.Require().NoError(err)

						// Check that the ibc/uatom are available
						osmoIBCAtom := s.EuropaChain.App.(*app.Europa).BankKeeper.GetBalance(s.EuropaChain.GetContext(), receiverAcc, teststypes.UatomOsmoIbcdenom)
					...
					It("should not recover tokens that originated from other chains", func() {
						...
						err := s.pathOsmosisEuropa.RelayPacket(CreatePacket("10000", constants.BaseDenom, sender, receiver, "transfer", "channel-0", "transfer", "channel-0", 1, timeout))
						s.Require().NoError(err)
						err = s.pathOsmosisEuropa.RelayPacket(CreatePacket("10", "transfer/channel-0/transfer/channel-1/uatom", sender, receiver, "transfer", "channel-0", "transfer", "channel-0", 2, timeout))
						s.Require().NoError(err)
						err = s.pathOsmosisEuropa.RelayPacket(CreatePacket("10", "transfer/channel-0/uosmo", sender, receiver, "transfer", "channel-0", "transfer", "channel-0", 3, timeout))
						s.Require().NoError(err)
