...
func (suite *IncentiveTestSuite) TestIncentiveNew() {
	testCases := []struct {
		...
		},
		{
			...
			utiltx.GenerateAddress(),
			sdk.DecCoins{{Denom: fmt.Sprintf("(%s", constants.BaseDenom), Amount: sdk.OneDec()}},
			10,
			false,
		},
		{
			...
			utiltx.GenerateAddress(),
			sdk.DecCoins{sdk.NewDecCoinFromDec(constants.BaseDenom, sdk.NewDecWithPrec(0, 2))},
			10,
			false,
		},
		{
			...
			utiltx.GenerateAddress(),
			sdk.DecCoins{sdk.NewDecCoinFromDec(constants.BaseDenom, sdk.NewDecWithPrec(101, 2))},
			10,
			false,
		},
		{
