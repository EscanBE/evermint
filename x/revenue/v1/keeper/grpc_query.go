...
func (k Keeper) Revenue(
	...
	if err := evertypes.ValidateNonZeroAddress(req.ContractAddress); err != nil {
...
func (k Keeper) DeployerRevenues( //nolint: dupl
	...
	if err != nil {
		return nil, status.Errorf(
			codes.InvalidArgument,
			"invalid format for deployer %s, should be bech32 ('%s1...')", req.DeployerAddress, constants.Bech32Prefix,
		)
	}
...
func (k Keeper) WithdrawerRevenues( //nolint: dupl
	...
	if err != nil {
		return nil, status.Errorf(
			codes.InvalidArgument,
			"invalid format for withdraw addr %s, should be bech32 ('%s1...')", req.WithdrawerAddress, constants.Bech32Prefix,
		)
	}
