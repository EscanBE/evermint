...
HTTPS_GIT := https://github.com/europa/europa.git
...
DEPS_COSMOS_ICS23 := go/$(shell cat go.sum | grep 'github.com/confio/ics23/go' | grep -v -e 'go.mod' | tail -n 1 | awk '{ print $$2; }')
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=europa \
...
build-docker:
	...
	$(DOCKER) rm europa || true
	# create a new container from the latest image
	$(DOCKER) create --name europa -t -i ${DOCKER_IMAGE}:latest europa
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp europa:/usr/bin/euscd ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/europa/europa"
	godoc -http=:6060
...
proto-download-deps:
	...
	curl -sSL "https://raw.githubusercontent.com/cosmos/ics23/$(DEPS_COSMOS_ICS23)/proofs.proto" > "$(THIRD_PARTY_DIR)/proofs.proto"
...
localnet-start: localnet-stop
	@if ! [ -f build/node0/$(APP_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/europa:Z europa/node "./euscd testnet init-files --v 4 -o /europa --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\build\node0\euscd:/europa\Z europa/node "./euscd tendermint unsafe-reset-all --home=/europa"
	@docker run --rm -v $(CURDIR)\build\node1\euscd:/europa\Z europa/node "./euscd tendermint unsafe-reset-all --home=/europa"
	@docker run --rm -v $(CURDIR)\build\node2\euscd:/europa\Z europa/node "./euscd tendermint unsafe-reset-all --home=/europa"
	@docker run --rm -v $(CURDIR)\build\node3\euscd:/europa\Z europa/node "./euscd tendermint unsafe-reset-all --home=/europa"
else
	@docker run --rm -v $(CURDIR)/build/node0/euscd:/europa:Z europa/node "./euscd tendermint unsafe-reset-all --home=/europa"
	@docker run --rm -v $(CURDIR)/build/node1/euscd:/europa:Z europa/node "./euscd tendermint unsafe-reset-all --home=/europa"
	@docker run --rm -v $(CURDIR)/build/node2/euscd:/europa:Z europa/node "./euscd tendermint unsafe-reset-all --home=/europa"
	@docker run --rm -v $(CURDIR)/build/node3/euscd:/europa:Z europa/node "./euscd tendermint unsafe-reset-all --home=/europa"
endif
...
PACKAGE_NAME:=github.com/europa/europa
